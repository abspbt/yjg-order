...
- 如果請求裡帶了 `pickup_slots`（陣列，可以是空陣列 `[]`），會拿新清單跟這個檔期原本的取貨時段比對（用 `date` + `time_range` 判斷是不是同一個時段）：**內容沒變的時段會保留原本的 `slot_id` 不變，只有真的新增/刪除的才會動**；沒帶這個欄位就完全不動取貨時段
  - ⚠️ **這是修過的行為，不要改回整批覆蓋**：舊版做法是不管內容有沒有變，只要帶了 `pickup_slots` 就把該檔期所有時段整批刪除、重新產生一批全新的 `slot_id`。老闆後台「編輯檔期」頁面每次儲存都會把目前的時段列表整包送出，就算只是改檔期名稱、總量上限這些跟時段完全無關的欄位也一樣，導致每次儲存都把已經送出訂單裡存的 `pickup_slot_id` 全部作廢、訂單的取貨時間憑空消失（實際發生過的資料遺失事故）
  - 如果新清單裡少了某個時段、而這個時段已經有訂單的 `pickup_slot_id` 指向它，會擋下來回 HTTP 400（比照 `DELETE /campaigns/:campaign_id` 的保護邏輯，不分訂單狀態，取消的訂單也算），不會讓那些訂單的取貨時間變成孤兒資料：
    ```json
    { "ok": false, "error": "以下取貨時段已經有訂單使用，無法刪除：2026-09-17 11:00~15:00，請保留這些時段，或改用其他日期/時段設定" }
    ```
